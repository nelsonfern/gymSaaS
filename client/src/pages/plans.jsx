...
export default function Plans(){
    ...
    return(
        ...
        <div className="mb-4 p-2">
            ...
            <p className="text-gray-500 text-sm font-normal">Aquí puedes ver todos los planes registrados en el gimnasio.</p>
        
        <div className="flex justify-end" >
            <button onClick={() => openSlide()} className="w-48 h-10 flex justify-center items-center align-center
        ...
        <div className="grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
            {plans.map((plan) => (
                <PlanCards plan={plan} isSelected={selectedPlan === plan._id}
            onClick={() => handleSelectPlan(plan._id)} 
            key={plan._id} name={plan.name} price={plan.price} duration={plan.durationDays} type={plan.typePlan} description={plan.description}/>
            ))}
        </div>
        <EditPlans />
        </>
